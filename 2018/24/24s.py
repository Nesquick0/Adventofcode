  ...
  def run(self):
    ...
    while (i<len(input)):
      ...
      if (m):
        ...
        if (infection):
          self.infectionOrig.append(group)
        else:
          self.immuneOrig.append(group)
        id += 1

      i += 1

    # Test different boost
    boost = 0
    success = False
    while (not success):
      self.immune = copy.deepcopy(self.immuneOrig)
      self.infection = copy.deepcopy(self.infectionOrig)
      for group in self.immune:
        group.dmg += boost
      ...
      while (len(self.infection) > 0 and len(self.immune) > 0):
        ...
        # Hotfix, check that nothing changed
        totalUnitsKilled = 0
